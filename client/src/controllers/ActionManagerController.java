...
public class ActionManagerController {
    ...
    public void Sellcars(){
        name.getScene().getWindow().hide();
        SceneLoaderInstance.INSTANCE.getInstance().loadScene("sellingcars", "Принять автомобиль");
    }
    public void ApproveALoan(){
        name.getScene().getWindow().hide();
        SceneLoaderInstance.INSTANCE.getInstance().loadScene("LoanManager", "Запросы на кредит");
    }
    public void logOut() {
        name.getScene().getWindow().hide();
